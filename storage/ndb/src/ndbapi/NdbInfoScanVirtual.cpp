...
int NdbInfoScanVirtual::readTuples() {
  ...
  if (m_state != Initial) DBUG_RETURN(NdbInfo::ERR_WrongState);
...
const NdbInfoRecAttr *NdbInfoScanVirtual::getValue(const char *anAttrName) {
  ...
  if (m_state != Prepared) DBUG_RETURN(NULL);

  const NdbInfo::Column *column = m_table->getColumn(anAttrName);
  if (!column) DBUG_RETURN(NULL);
...
const NdbInfoRecAttr *NdbInfoScanVirtual::getValue(Uint32 anAttrId) {
  ...
  if (anAttrId >= m_table->columns()) DBUG_RETURN(NULL);
...
class VirtualTable {
  ...
  */
  ...
  /*
    ...
    >1 Error occurred
  */
  virtual int read_row(VirtualScanContext *ctx, Row &row,
                       Uint32 row_number) const = 0;

  int seek(std::map<int, int>::const_iterator &, const int &key,
           NdbInfoScanOperation::Seek) const;

  virtual ~VirtualTable() = 0;

  static constexpr int STRING_BUFFER = 516;  // 512 + final null + padding
...
int index_seek(const std::map<int, int> &index,
               std::map<int, int>::const_iterator &iter, const int &key,
  ...
  if (!(seek.inclusive() && (iter->first == key))) {
    ...
    if (seek.high() || seek.low()) {
      ...
      return -1;  // exact match failed and ranges are not wanted
  }

  return (iter == index.cend()) ? -1 : iter->second;
}

int VirtualTable::seek(std::map<int, int>::const_iterator &iter, const int &key,
...
bool VirtualTable::Row::check_buffer_space(size_t len) const {
  if (m_curr + len > m_end) {
...
VirtualTable::Row::Row(NdbInfoScanVirtual *owner, const NdbInfo::Table *table,
                       char *buffer, size_t buf_size)
    : m_owner(owner),
      m_table(table),
...
int NdbInfoScanVirtual::nextResult() {
  ...
  if (m_state != MoreData) DBUG_RETURN(-1);

  assert(m_buffer);

  VirtualTable::Row row(this, m_table, m_buffer, m_buffer_size);
...
class VirtualScanContext {
 public:
  ...
  const char *getDatabaseName() const { return m_ndb->getDatabaseName(); }
  ...
  */
  ...
  int seek(std::map<int, int>::const_iterator &iter, const int &key,
           NdbInfoScanOperation::Seek seek) const {
...
int NdbInfoScanVirtual::execute() {
...

  {
    ...
    for (unsigned i = 0; i < m_table->columns(); i++) {
      if (!m_recAttrs.is_requested(i)) continue;
    ...
    if (!m_buffer) DBUG_RETURN(NdbInfo::ERR_OutOfMemory);
    m_buffer_size = buffer_size;
  }

  if (!m_virt->start_scan(m_ctx)) DBUG_RETURN(NdbInfo::ERR_VirtScanStart);
...
int NdbInfoScanVirtual::init() {
  if (m_state != Undefined) return NdbInfo::ERR_WrongState;

  m_state = Initial;
  return NdbInfo::ERR_NoError;
}

void NdbInfoScanVirtual::initIndex(Uint32) { m_ctx->buildIndex(); }
 ...
 public:
  ...
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab->addColumn(
        ...
        !tab->addColumn(
            NdbInfo::Column("block_name", 1, NdbInfo::Column::String)))
      return nullptr;
    return tab;
  }
};
...
class DictObjTypesTable : public VirtualTable {
 ...
 public:
  ...
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab) return nullptr;
    if (!tab->addColumn(
            NdbInfo::Column("type_id", 0, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("type_name", 1, NdbInfo::Column::String)))
      return nullptr;
    return tab;
  }
};
 ...
 public:
  bool init() override {
    ...
    for (int i = 0; i < ndb_mgm_noOfErrorMsgs; i++) {
      ...
      if (m_error_messages.push_back(err) != 0) return false;
    }
    ...
    for (int i = 0;; i++) {
      ...
      const char *class_msg;
      const char *error_msg;
      if (ndb_error_get_next(i, &err_no, &status_msg, &class_msg, &error_msg) ==
          -1) {
        // No more NDB errors
        break;
      }
    ...
    for (int i = 0;; i++) {
      ...
      const char *class_msg;
      const char *error_msg;
      if (ndbd_exit_code_get_next(i, &exit_code, &status_msg, &class_msg,
                                  &error_msg) == -1) {
  ...
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab) return nullptr;
    if (!tab->addColumn(
            NdbInfo::Column("error_code", 0, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("error_description", 1, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("error_status", 2, NdbInfo::Column::String)) ||
...
class ConfigParamsTable : public VirtualTable {
 ...
 public:
  bool init() override {
    ...
    ConfigInfo::ParamInfoIter param_iter(m_config_info, CFG_SECTION_NODE,
                                         NODE_TYPE_DB);
    int row_count = 0;
    ...
    while ((pinfo = param_iter.next())) {
      ...
      if (m_config_params.push_back(pinfo) != 0) return false;
  ...
  int read_row(VirtualScanContext *, VirtualTable::Row &w,
    ...
    switch (param->_type) {
      case ConfigInfo::CI_BOOL: {
        ...
        if (info.hasDefault(section, param_name))
          BaseString::snprintf(tmp_buf, sizeof(tmp_buf), "%llu",
      ...
      case ConfigInfo::CI_INT64: {
        ...
        // param_min
        BaseString::snprintf(tmp_buf, sizeof(tmp_buf), "%llu",
                             info.getMin(section, param_name));
        w.write_string(tmp_buf);
    ...
    // param_mandatory
    Uint32 mandatory = 0;
    if (info.getMandatory(section, param_name)) mandatory = 1;
    w.write_number(mandatory);

    // param_status
    const char *status_str = "";
    Uint32 status = info.getStatus(section, param_name);
    if (status & ConfigInfo::CI_EXPERIMENTAL) status_str = "experimental";
    if (status & ConfigInfo::CI_DEPRECATED) status_str = "deprecated";
  ...
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab) return nullptr;
    if (!tab->addColumn(
            NdbInfo::Column("param_number", 0, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("param_name", 1, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("param_description", 2, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("param_type", 3, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("param_default", 4, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("param_min", 5, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("param_max", 6, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("param_mandatory", 7, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("param_status", 8, NdbInfo::Column::String)))
...
class NdbkernelStateDescTable : public VirtualTable {
 ...
 public:
  ...
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab) return nullptr;
    if (!tab->addColumn(
            NdbInfo::Column("state_int_value", 0, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("state_name", 1, NdbInfo::Column::String)) ||
        !tab->addColumn(NdbInfo::Column("state_friendly_name", 2,
                                        NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("state_description", 3, NdbInfo::Column::String)))
      return nullptr;
    return tab;
  }
};
 ...
 public:
  ...
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab->addColumn(NdbInfo::Column("id", 0, NdbInfo::Column::Number64)))
      return nullptr;
    if (!tab->addColumn(
            NdbInfo::Column("fragment", 1, NdbInfo::Column::Number)))
      return nullptr;
    if (!tab->addColumn(
            NdbInfo::Column("row_id", 2, NdbInfo::Column::Number64)))
      return nullptr;
    return tab;
  }
};
 ...
 public:
  ...
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab->addColumn(
      ...
      return nullptr;
    if (!tab->addColumn(
            NdbInfo::Column("index_version", 1, NdbInfo::Column::Number)))
      return nullptr;
    if (!tab->addColumn(
            NdbInfo::Column("sample_version", 2, NdbInfo::Column::Number)))
      return nullptr;
    if (!tab->addColumn(
            NdbInfo::Column("load_time", 3, NdbInfo::Column::Number)))
      return nullptr;
    if (!tab->addColumn(
            NdbInfo::Column("sample_count", 4, NdbInfo::Column::Number)))
      return nullptr;
    return tab;
  }
};
 ...
 public:
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab->addColumn(
        ...
        !tab->addColumn(
            NdbInfo::Column("table_name", 1, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("table_id", 2, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("status", 3, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("attributes", 4, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("primary_key_cols", 5, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("primary_key", 6, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("storage", 7, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("logging", 8, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("dynamic", 9, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("read_backup", 10, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("fully_replicated", 11, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("checksum", 12, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("row_size", 13, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("min_rows", 14, NdbInfo::Column::Number64)) ||
        !tab->addColumn(
            NdbInfo::Column("max_rows", 15, NdbInfo::Column::Number64)) ||
        !tab->addColumn(
            NdbInfo::Column("tablespace", 16, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("fragment_type", 17, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("hash_map", 18, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("fragments", 19, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("partitions", 20, NdbInfo::Column::Number)) ||
        !tab->addColumn(NdbInfo::Column("partition_balance", 21,
                                        NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("contains_GCI", 22, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("single_user_mode", 23, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("force_var_part", 24, NdbInfo::Column::Number)) ||
        !tab->addColumn(
  ...
  int read_row(VirtualScanContext *ctx, VirtualTable::Row &w,
...

    {
      ...
      cstrbuf<column_buff_size> pk_cols;
      ...
      for (int i = 0; i < t->getNoOfPrimaryKeys(); i++) {
        BaseString col = quoteColumnName(t->getPrimaryKey(i));
        if (pk_cols.length() + 1 + col.length() < TRUNCATE_AT) {
          if (i) pk_cols.append(",");
          pk_cols.append(col.c_str());
        } else {
          pk_cols.append(TRUNCATION_MARK);
          break;
        }
      }
      ...
      w.write_string(pk_cols.c_str());                 // primary_key
 ...
 public:
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab->addColumn(
        ...
        !tab->addColumn(
            NdbInfo::Column("database_name", 1, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("table_name", 2, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("column_id", 3, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("column_name", 4, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("inline_size", 5, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("part_size", 6, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("stripe_size", 7, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("blob_table_name", 8, NdbInfo::Column::String)))
      return nullptr;
    return tab;
  }
 ...
 public:
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab->addColumn(
        ...
        !tab->addColumn(NdbInfo::Column("name", 1, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("table_id", 2, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("reporting", 3, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("columns", 4, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("table_event", 5, NdbInfo::Column::Number)))
      return nullptr;
    return tab;
  }
  ...
  int read_row(VirtualScanContext *ctx, VirtualTable::Row &w,
    ...
    {
      cstrbuf<column_buff_size> event_columns;
      Uint32 reporting = 0;
      Uint32 table_event = 0;

      /* columns */
      for (int i = 0; i < event->getNoOfEventColumns(); i++) {
        BaseString col = quoteColumnName(event->getEventColumn(i)->getName());
        if (event_columns.length() + 1 + col.length() < TRUNCATE_AT) {
          if (i) event_columns.append(",");
          event_columns.append(col.c_str());
        } else {
          event_columns.append(TRUNCATION_MARK);
          break;
        }
      }
      ...
      */
      ...
      w.write_string(event_columns.c_str());             // columns
...
class IndexColumnsTable : public VirtualTable {
 public:
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab->addColumn(
        ...
        !tab->addColumn(
            NdbInfo::Column("table_name", 2, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("index_object_id", 3, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("index_name", 4, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("index_type", 5, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("status", 6, NdbInfo::Column::Number)) ||
        !tab->addColumn(NdbInfo::Column("columns", 7, NdbInfo::Column::String)))
      return nullptr;
    return tab;
  }
  ...
  int read_row(VirtualScanContext *ctx, VirtualTable::Row &w,
               Uint32) const override {
    cstrbuf<column_buff_size> index_columns;
    ...
    for (unsigned int i = 0; i < index->getNoOfColumns(); i++) {
      BaseString col = quoteColumnName(index->getColumn(i)->getName());
      if (index_columns.length() + 1 + col.length() < TRUNCATE_AT) {
        if (i) index_columns.append(",");
        index_columns.append(col.c_str());
      } else {
        index_columns.append(TRUNCATION_MARK);
        break;
      }
    }
    ...
    w.write_string(index_columns.c_str());          // columns
 ...
 public:
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab->addColumn(
        ...
        !tab->addColumn(NdbInfo::Column("name", 1, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("parent_table", 2, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("parent_columns", 3, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("child_table", 4, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("child_columns", 5, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("parent_index", 6, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("child_index", 7, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("on_update_action", 8, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("on_delete_action", 9, NdbInfo::Column::Number)))
      return nullptr;
    return tab;
  }
  ...
  int read_row(VirtualScanContext *ctx, VirtualTable::Row &w,
               Uint32 row) const override {
    cstrbuf<column_buff_size> parent_columns;
    cstrbuf<column_buff_size> child_columns;
    ...
    for (unsigned int i = 0; i < fk.getParentColumnCount(); i++) {
      if (i) parent_columns.append(",");
      parent_columns.appendf("%d", fk.getParentColumnNo(i));
    }

    /* child_columns */
    for (unsigned int i = 0; i < fk.getChildColumnCount(); i++) {
      if (i) child_columns.append(",");
      child_columns.appendf("%d", fk.getChildColumnNo(i));
    }
    ...
    w.write_string(parent_columns.c_str());      // parent_columns
    w.write_string(fk.getChildTable());          // child_table
    w.write_string(child_columns.c_str());       // child_columns
 ...
 public:
  NdbInfo::Table *get_instance() const override {
    ...
    if (!tab->addColumn(
            NdbInfo::Column("table_id", 0, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("column_id", 1, NdbInfo::Column::Number)) ||
        !tab->addColumn(NdbInfo::Column("name", 2, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("column_type", 3, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("default_value", 4, NdbInfo::Column::String)) ||
        !tab->addColumn(
            NdbInfo::Column("nullable", 5, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("array_type", 6, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("storage_type", 7, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("primary_key", 8, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("partition_key", 9, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("dynamic", 10, NdbInfo::Column::Number)) ||
        !tab->addColumn(
            NdbInfo::Column("auto_inc", 11, NdbInfo::Column::Number)))
      return nullptr;
    return tab;
  }
...
*/
bool NdbInfoScanVirtual::create_virtual_tables(Vector<NdbInfo::Table *> &list) {
  // polymorphic lambda:
  ...
  return (add_table(new BlocksTable) && add_table(new DictObjTypesTable) &&
          add_table(new ErrorCodesTable) && add_table(new ConfigParamsTable) &&
          add_table(new NdbkernelStateDescTable(
              "dbtc_apiconnect_state", g_dbtc_apiconnect_state_desc)) &&
          add_table(new NdbkernelStateDescTable(
              "dblqh_tcconnect_state", g_dblqh_tcconnect_state_desc)) &&
          add_table(new BackupIdTable) && add_table(new IndexStatsTable) &&
          add_table(new DictionaryTablesTable) && add_table(new BlobsTable) &&
          add_table(new EventsTable) && add_table(new IndexColumnsTable) &&
          add_table(new ForeignKeysTable) && add_table(new ColumnsTable));
}
